...
## 11. 「上传主题包 / 可插拔主题」可行性评估（RFC，2026-06-13）

> 对应 §8.3 后续建议第 4 条（主题市场 / 上传安装）。维护者设想：「人人都能做主题，后台上传 GitHub 压缩包就新增主题」。
> 结论：愿景好，但"上传压缩包 → 运行期新增主题"与当前架构和北极星天然冲突；现实分三档，**推荐先做 T1**。

### 11.1 当前架构真相：构建期装配，运行期只是"选"
- 主题 = **编译进 Next bundle 的 React 组件**。`themes/index.ts` 是写死的注册表 + 静态/动态 `import`，webpack 在构建期就把每套主题切成 chunk；`components/Layout/index.tsx`（§6）更是**静态 import** `defaultTheme`/`novaTheme` 进 `themesComponents` 映射。
- 运行期的 `themeName`（后台 setting → `/api/public/meta` → LayoutProps）只是**选一套早已打进镜像的主题**，没有任何"注册新主题"的能力；名字不在表里就 fallback default。

### 11.2 核心冲突：重启 ≠ 重建
- 线上是 `Dockerfile` RUNNER 阶段的 **Next standalone 产物**——只 `COPY` 了 `.next/standalone` + `.next/static` + `public`，**没有主题源码、没有 `next` 构建链**。重启 = 重跑 `node server.js` 服务已编译的 `.next`，**不会** 重新 `next build`。
- 所以「后台脚本丢主题 + 重启**同一个镜像**」→ **新主题不会出现**：镜像里既无源码也无工具链，重启也不触发编译。
- 即便想"启动时构建"：`getStaticProps` 构建期要 fetch 文章 → 必须先起 NestJS+Mongo；且 slim 镜像本就没工具链。

### 11.3 三条现实路径
| 档 | 做法 | 离愿景 | 代价/风险 | 保北极星 |
|---|---|---|---|---|
| **T1 契约化 + PR 主题** | 冻结 `ThemeComponents` 接口 + 脚手架 + 文档，照模板写 `themes/xxx/` 提 PR、随镜像构建 | 80% | 🟢 极低，架构几乎不动 | ✅ 完全 |
| **T2 后台触发重建** | 后台/脚本把主题入仓 → 触发 **CI 重建** → 服务器拉新镜像重启 | 接近（几分钟，非秒级） | 🟠 几分钟构建 + 一次重部署 | ⚠️ 站本身快，但"加主题"重 |
| **T3 真·运行期可插拔** | (a) Module Federation 预编译远程 JS；(b) 模板引擎（类 WordPress/Ghost）运行期解释 | 100% | 🔴 大改；(b) 折损 SSG | (a) 勉强 (b) 多半折损 |

> 「后台脚本 + 重启」唯一能落地的形态 = **T2**——这里的"重启"其实是"拉一个 CI 重建过的新镜像再起"，**不是绕开重建**。

### 11.4 风险
- **安全**：上传并执行任意 GitHub 代码 = **RCE / 供应链**风险；做成社区市场尤甚。
- **定位漂移**：本 fork 自定位"**保守维护 + 与上游同构**"，做成主题平台会拉大分叉、越来越难合上游。需想清楚要"一个很快的博客"还是"一个主题平台"。

### 11.5 建议（分阶段，别一步到位）
1. **先做 T1**（零架构风险、完全保北极星，把"人人能做主题" 80% 落地）：
   - `types.ts` 的松散 `[key: string]` 收紧成**必填键接口**（主题少实现一个组件即编译报错）；
   - `index.ts` 的写死注册表 → 构建期 glob 自动发现 `themes/*/theme.ts`（"加主题 = 丢个文件夹 + 重建"）；顺手把 `Layout/index.tsx` 的静态映射也改成走 `loadTheme`/自动发现，消除"两处登记主题"的重复；
   - 出 `themes/_template/` 脚手架 + "如何做一套主题"指南 + 每套 `theme.json`（名/作者/预览图）。
2. 真要"后台上传"再上 **T2**，且只对自托管站长开放、明确告知"新增主题需几分钟重建"。
3. **T3 除非把项目定位从"博客"转向"主题平台"，否则 ROI 不划算。**

---

**文档版本**: 1.3.0 **创建日期**: 2026-04-19 **最后更新**: 2026-06-13 **更新内容**:

- 完成 Nova 主题集成
- 添加完整文件变更清单
- 添加测试验证清单
- 新增 §11：「上传主题包 / 可插拔主题」可行性评估 RFC（构建期 vs 运行期、重启≠重建、T1/T2/T3 分档与建议）
