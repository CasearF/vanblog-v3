...
> 另案 backlog:① ~~Caddy 2.8+ 配置现代化~~ ✅ **已落地(2026-06-16,升 2.8.4,分支 `chore/caddy-2.8-modernize`)**:经 2.8.4 源码核实,实际只需 `caddyTemplate.json` 的 `on_demand.ask` → `permission`(http 模块)一处;`trusted_proxies` 裸数组在 2.8 仍合法、server `CaddyProvider` 零改动(本行原说的「trusted_proxies 改模块 + 双改」有误,详见 [deployment-and-ci-notes](./deployment-and-ci-notes.md) §11);② Node 24(需 cli@11 等框架升级,属 Tier 2/3)。下面自「两项开工前待定」起的内容是当时按 Node 24 写的历史记录,已被本结论取代。
