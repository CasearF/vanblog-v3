...
- [x] Caddy 现代化（2.6.4 → 2.8.4）：解除 Tier 1 的临时钉版，将 `on_demand.ask` 迁移到 `permission`（http 模块，2.8 起的新格式）；仍采用钉死的静态二进制（与 Alpine 基底漂移解耦、镜像可复现），迁移后经 Caddy 2.8.4 本机 `validate` 通过（详见维护笔记）
- [x] 运行时 / 包管理升级（Tier 1）：Node 18（已 EOL）→ 22 LTS、pnpm 8.11 → 9.15.9、补 `engines` / `.nvmrc` 钉版、`pnpm-lock` 迁移至 v9；Caddy 临时钉定 2.6.4（规避 Alpine 基底漂移；后经上条 Caddy 现代化升至 2.8.4）；并清理无人构建的遗留 Dockerfile（详见维护笔记）
